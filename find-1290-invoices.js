...
async function findInvoicesFor1290() {
  ...
  try {
    ...
    console.log(
      `Invoices with DocNumber starting with "1290": ${invoices1290.length}\n`
    );
    ...
    if (invoices1290.length > 0) {
      ...
      allInvoices.slice(0, 20).forEach((inv) => {
        console.log(
          `  ${inv.DocNumber} (Customer: ${inv.CustomerRef?.name || inv.CustomerRef?.value})`
        );
      });
    }
    ...
    if (job2927Invoices.length > 0) {
      job2927Invoices.forEach((inv, idx) => {
        console.log(
          `  ${idx + 1}. DocNumber: ${inv.DocNumber}, Amount: ${inv.TotalAmt}`
        );
      });
    }
  } catch (error) {
