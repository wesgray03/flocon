...
async function runMigration() {
  ...
  // Read the SQL file
  const sqlPath = path.join(__dirname, '2025-11-18-create-qbo-vendor-import-list.sql');
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    try {
      ...
      if (error) {
        ...
        const { error: directError } = await supabase.from('qbo_vendor_import_list').select('id').limit(1);
